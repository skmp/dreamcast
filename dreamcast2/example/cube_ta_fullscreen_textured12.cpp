...
void transfer_ta_cube(float intensity, uint32_t texture_address)
{
  {
  ...
  for (int face_ix = 0; face_ix < cube_faces_length; face_ix++) {
    ...
    // printf("i: %f, intensity: %f, tonal_offset: %f\n", i, intensity, tonal_offset);
    
    float i2 = i * 255.0f;
    if (i2 < 0.0f) i2 = 0.0f;
    if (i2 > 255.0f) i2 = 255.0;
...
int main()
{
  ...
  */
  ...
  for(;;) {
    ...
    transfer_ta_cube(shinnyiness < 127 ? 0.2f : 1.2f, texture_start);
